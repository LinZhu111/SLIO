...
class SNN_ConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, norm=None, tau=2.0, v_threshold=1.0, v_reset=None, activation_type = 'lif'):
        ...
        if activation_type == 'lif':
            self.activation = MpSpikeLIFNode(v_threshold=v_threshold, tau = tau, v_reset=v_reset, surrogate_function=surrogate.ATan())
        elif activation_type == 'if':
            self.activation = MpSpikeIFNode(v_threshold=v_threshold, v_reset=v_reset, surrogate_function=surrogate.ATan())
        elif activation_type == 'plif':
            self.activation = MpSpikeParametricLIFNode(v_threshold=v_threshold, v_reset=v_reset, surrogate_function=surrogate.ATan())
        self.norm_layer = nn.BatchNorm2d(out_channels)
        
        self.mp_activation = MpIFNode(v_threshold=v_threshold, tau = tau, v_reset=v_reset, surrogate_function=surrogate.ATan())
        
    def forward(self, x):
        out = self.conv2d(x)
        out = self.norm_layer(out)
        out, v = self.activation(out)
        return out, v

class SNN_ConvLayer_MP(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, norm=None, tau=2.0, v_threshold=1.0, v_reset=None, activation_type = 'lif'):
        super(SNN_ConvLayer_MP, self).__init__()
        ...
        self.mp_activation = MpIFNode(v_threshold=v_threshold, v_reset=v_reset, surrogate_function=surrogate.ATan())
        
    def forward(self, x):
        out = self.conv2d(x)
        out = self.norm_layer(out)
        s, v = self.activation(out)
        mp = self.mp_activation(out, None)
        return s, v, mp
